...
function recommendMovie(selectedTitle) {
  ...
  // 计算每部电影与所选电影共享类型的数量，评分差异，和时长差异
  const recommendations = movies.filter(movie => movie.title !== selectedTitle)
      .map(movie => {
          const sharedGenresCount = movie.genre.reduce((count, genre) => {
              return count + (selectedGenres.includes(genre) ? 1 : 0);
          }, 0);
          const ratingDiff = Math.abs(movie.rating - selectedMovie.rating);
          const durationDiff = Math.abs(movie.duration - selectedMovie.duration) / selectedMovie.duration;

          // 加权计分: 类型匹配权重最大，其次是评分接近，最后是时长接近
          const score = sharedGenresCount * 3 - (ratingDiff > 0.5 ? 5 : 0) - (durationDiff > 0.2 ? 5 : 0);
          return {
              movie,
              score
          };
      })
      .filter(item => item.score > 0) // 过滤掉得分为零或负分的电影
      .sort((a, b) => b.score - a.score) // 根据得分降序排序
      .slice(0, 3) // 选择得分最高的前三部电影
      .map(item => item.movie); // 从包含评分的对象中提取电影对象
